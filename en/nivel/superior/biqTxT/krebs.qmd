...
1. Saber a localização distinta do ciclo de Krebs em relação à glicólise;
1. Entender a lógica cíclica deste circuito metabólico;
1. Saber exemplificar a necessidade de algumas vitaminas do complexo B, tais como B1, B2 e B3 para o correto funcionamento do circuito (coenzimas);
1. Compreender e saber exemplificar a natureza anfibólico do ciclo de Krebs;
1. Entender a produção de coenzimas de oxi-redução (NADH e FADH2) em suas desidrogenases respectivas;
1. Compreender a descarboxilação dupla que ocorre neste circuito;
1. Saber exemplificar os controles metabólicos alostérico e covalente envolvidos no circuito;
1. Entender como um atalho no circuito pode poupar carbonos da descarboxilação (ciclo do glioxilato);
1. Entender a produção de GTP direta no circuito;
1. Saber o saldo de coenzimas de oxiredução do circuito;
1. Observar como o ciclo de Krebs está interligado a outras rotas metabólicas.
...
1. Conceito: via cíclica de restauração de ácido cítrico a partir de acetil CoA, para a produção de metabólitos utilizados em outras vias;
1. Importância: permite a produção de coenzimas (NADH e FADH2) que serão utilizadas na cadeia respiratória para a produção de ATP, e fornece intermediários de outros ciclos, como oxalacetato e glutamato;
1. Localização: mitocôndria (matriz);
1. Lógica da via: condensação de C2 com C4 (para produzir C6) e desidrogenações (para produzir C4 e as coenzimas), restaurando C6 em nova condensação (descarboxilação de 2 CO2 permite retorno C6 para C4 do ciclo);
1. Principais vitaminas da via: vitamina B3, ácido lipóico, vit B1, vit B2;
1. Descarboxilações e desidrogenações ( produzindo CO2, 3 NADH, 1 FADH2 e 1 GTP ou ATP);
1. Regulação por fofoenolpiruvato (+), acil CoA (-) e NADH (-);
1. Inibições por malonato e arsenato sobre algumas enzimas da via; esta interrupção promove a inativação da via, e consequente estagnação do metabolismo intermediário;
1. Doença: béribéri (carência de vitamina B1, levando a quadro neurológico e cardiovascular).
