...
![Mecanismo de ação das enzimas. A catálise é efetivada por redução da energia de ativação necessária para a condução de uma reação química. Notar que a energia livre de Gibbs do produto é inferior a do substrato.](enz-therm.png){#fig-enz-therm}


|       A @fig-enz-therm ilustra o mecanismo de ação das enzimas. Neste diagrama, as coordenadas indicam o curso da reação (X), a energia envolvida na reação (Y), a variação de energia sem a enzima ($\Delta$G$_{nãocat}$), a variação de energia na presença da enzima ($\Delta$G$_{cat}$), o substrato S, o complexo enzima-substrato ES, o produto P, e o *estado de transição* (‡). Esse último representa o montante de energia necessária para que uma reação ocorra, quer na ausência ou na presença da enzima. 

|       Neste último caso, diz-se que a enzima estabiliza o estado de transição do substrato (transição, significando o equilíbrio S<-->P). Metaforicamente, o diagrama da @fig-enz-therm pode ser representado como um ciclista (S) buscando chegar ao seu destino (P). Ele pode subir um morro único bastante íngreme (reação não catalisada), gastando para isso muita energia, ou pode pedalar por três pequenos morrinhos (reação catalisada), poupando esforço. 
