...
| Despite its ease of use with a mouse, without the need to enter text commands, *Sisma* allows all elements of the map (objects, paths, equations, names, colors, positions) to be modified by editing the map file in Notepad, adding value to *Reproducible Teaching* with the tool. The figure below illustrates a *Sisma* file opened in a notepad for editing.
\
