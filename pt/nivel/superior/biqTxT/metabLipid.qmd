...
1. Localizar no mapa metabólico a síntese e a degradação de ácidos graxos, bem como o metabolismo do colesterol;
1. Entender que a síntese e a degradação de ácidos graxos se dá de forma espiralar, adicionando ou retirando, respectivamente, grupos acetato;
1. Compreender como a acetil CoA mitocondrial se regenera no citosol, sítio de síntese de AG.

#### Síntese de ácidos graxos {.unnumbered}

1. Entender que a síntese de colesterol é muito maior do que a de ácidos graxos em humanos, o que privilegia o controle metabólico do primeiro;
1. Compreender a ação da acetil carboxilase e sua modulação na síntese de ácidos graxos;
1. Saber qual o papel da ACP (proteína carreadora de AG) e da sintase de ácidos graxos;
1. Compreender o papel da biotina e de NADPH na síntese de AG;
1. Entender como se processa espiral de síntese de AG, compreendendo as etapas de redução, desidratação e redução novamente.


#### Degradação de ácidos graxos {.unnumbered}

1. Compreender que a degradação ocorre em duas etapas, a mobilização dos estoques de ácidos graxos dos tecidos (triacilglicerol), lipólise, e a beta-oxidação de ácidos graxos mitocondrial;
1. Compreender o papel da lipase sensível a hormônio (glicagon, epinefrina) e da albumina sérica na lipólise;
1. Compreender a necessidade de carnitina à beta-oxidação de AG;
1. Entender o processo em espiral reverso ao da síntese de AG: oxidação, hidratação, oxidação;
1. Entender que cada volta na espiral da beta-oxidação libera 1 acetil CoA, 1 FADH2, e 1 NADH;
1. Saber que a beta-oxidação de AG é um espelho metabólico do terço final do ciclo de Krebs.

#### Metabolismo de corpos cetônicos {.unnumbered}

1. Saber o que são os corpos cetônicos e sua função (acetoacetato, beta-hidroxibutirato; segunda fonte de energia depois da glicose;
1. Entender porque esses compostos podem ser utilizados como fontes de energia (acetil CoA);
1. Compreender que a descarboxilação acetoacetato produz acetona, e ocorre espontaneamente em humanos;
1. Compreender que o excesso metabólico de corpos cetônicos tende a reduzir o pH sanguíneo;
1. Saber que o acetil CoA é precursor de corpos cetônicos, quer pelo excesso, quer pela beta-oxidação de ácidos graxos.
...
![Resumo das interações entre o metabolismo intermediário e a síntese e degradação de ácidos graxos.](lipidMetResum.png){#fig-lipidMetResum}
...
![Rotas bioquímicas da síntese de ácidos graxos.](agSintese.png){#fig-agSintese}
...
![Rotas bioquímicas de degradação de ácidos graxos ($\beta$-oxidação).](agDegrad.png){#fig-agDegrad}
...
#### Biossíntese {.unnumbered}
1. Conceito: via cíclica de produção de ácido graxo a partir de ligações sucessivas de acetil CoA; 
1. Importância: produção de gorduras (triacilglicerol) utilizáveis pelo organismo para suprimento energético; 1. Localização: citosol;
1. Produção de ácidos graxos de cadeia ímpar: síntese de ácido propiônico: consumo de ATP, e vitaminas biotina e vit. B12;
1. Doença: acidúria metilmalônica (incapacidade de
conversão de vit B12);
1. Lógica da via: ancoramento de precursor (acetato-C2, mais comum, ou propionato-C3) em proteína
transportadora de grupos acila (ACP), transacilação (liberação de C2 ligado em outro sítio da proteína), síntese (montagem sequencial de C2-C2-C2-etc), e esterificação (liberação do ácido graxo); posteriormente, alongamento e dessaturação;
1. Transporte mitocondrial de acetil CoA: necessidade de condensação de acetil CoA e oxalacetato em citrato (citrato sintase), exportado para o citosol, e novamente reoxidado (citrato liase);
1. Primeira etapa: conversão de acetil CoA em malonil CoA por acetil CoA carboxilase, em presença de biotina;
1. Principal enzima: complexo multicatalítico da sintase dos ácidos graxos; nos procariontes, papel da ACP (domínio de fosfopanteteína);
1. Principais tecidos: fígado e glândulas mamárias (lactação);
1. Fontes: via das pentoses e oxidação de malato;
1. Regulação: adrenalina (-), citrato (+), insulina (+), jejum prolongado (-);
1. Síntese de triacilglicerol: produção de glicerol fosfato hepático ou adiposo a partir de diidroxiacetona fosfato, ou pela glicerol quinase (somente fígado); posteriormente,
atividade da aciltransferase dos ácidos graxos;
1. Consumo de 2 NADPH por volta no ciclo

#### Degradação

1. Conceito: liberação sequencial de acetil CoA do ácido graxo; 
1. Importância: síntese de grande quantidades de ATP;
1. Localização: mitocôndria (matriz) de tecido adiposo marrom;
1. Transporte mitocondrial (carnitina leva ácido graxo do citosol à matriz mitocondrial);
1. Rendimento de ATP: 1 acetil CoA, 1 FADH2 e 1 NADH por volta no ciclo; ex: palmitato,
16C, percorre 7 vezes a espiral até o último acetil CoA, produzindo 7 FADH2, 7 acetil CoA, 7 NADH, e 1 último acetil CoA. Como cada acetil CoA rende 12 ATPs no ciclo de Krebs, cada NADH rende 3 ATPs na cadeia respiratória, e cada FADH2 rende 2 ATPs na mesma cadeia, a oxidação do palmitato produz 7x3+7x2+8x12 ATPs. Total: 131 ATPs;
1. Lógica da via: liberação em espiral de acetil CoA;
1. Regulação: malonil CoA (-) e acetil CoA (-), lipase sensível a hormônio (+);
1. Vitaminas do ciclo: niacina (NAD) e riboflavina (FAD);

#### Corpos cetônicos

1. Conceito: formação de acetoacetato e hidroxibutirato a partir de acetil CoA em excesso; 1. Importância: podem migrar pelo sangue, podendo ser novamente re-sintetizados a acetil CoA e utilizados como combustíveis em tecidos musculares,cérebro e córtex adrenal; em excesso, causam acidose, podendo levar à morte;
1. Doenças: cetoacidose diabética (hiperglicemia, cetonemia e cetonúria, devido a alta
relação glicagon/insulina, com consequente ativação da beta-oxidação mediada por AMPc); síndrome da morte súbita do lactente (quadro de hipoglicemia severa, por
deficiência da acil CoA desidrogenase - 1a enzima da oxidação).
