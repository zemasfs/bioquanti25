...
|       A inibição de enzimas por moléculas que não o próprio substrato pode ser representada por um diagrama no qual o efetor altera um dos elementos representados na @eq-diagMM, tal como na figura abaixo.
...
|       Observe que os termos multiplicadores inseridos em *S* e *Km* na equação de duplo-recíproco apenas alteram seu formalismo apresentado @eq-eqLB. Dessa forma, os modelos de inibição enzimática podem ser ilustrados pelo `R` junto à transformação de Lineweaver-Burk (ou qualquer outra), como abaixo.
